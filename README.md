...
| **流量监控** | 查看实时速率、历史趋势、日 / 周 / 月累计流量，配置流量告警，导出 CSV / JSON 数据（可选今日 / 本周 / 本月 / 全部历史范围，含带时间戳的历史采样）。 |
...
运行日志写在同一目录的 `logs/` 子目录下（`netassist.log.按天日期`），按天滚动、**保留最近 7 天**，自动清理过期文件。提交问题时可直接附上其中近几天的文件；日志只记录应用自身的运行事件，不会记录你的流量内容。
...
反馈问题时，建议附上应用版本、操作系统与架构、复现步骤、预期和实际结果；涉及诊断报告或日志时，请先隐去不希望公开的 IP、进程信息等内容。运行日志位于配置目录的 `logs/` 子目录（见上文「数据存储与联网行为」），可直接附上对应日期的文件。
