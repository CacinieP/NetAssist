...
> 本报告为架构与设计层面的说明，已实现能力以当前代码、README 与各版本 Release 说明为准。
...
4. **日志与持久化**
   - 运行日志经 `tracing-appender` 按天滚动写入配置目录 `logs/`，非阻塞写入不占用 UI 线程
   - 保留最近 7 天，启动时与每日翻篇时自动清理过期文件（含对无日期后缀文件的防御性跳过）
   - release 构建仅落文件（打包后 stdout 被系统丢弃），debug 构建同时保留终端输出
   - 流量历史按天文件存储、原子写（临时文件 + 重命名）、45 天滚动保留，与日志清理同属启动/日翻篇兜底策略
...
> 方向性计划，具体进展以各版本 Release 与 CHANGELOG 为准。

### 已落地（截至 v0.3.6，2026-09）

- 网络使用历史趋势图表（1 小时 ~ 30 天多粒度）
- 流量告警与限额（日 / 周 / 月），累计流量基于 OS 接口计数器 + 磁盘锚点
- 深色模式主题、中英双语切换（i18next）
- 断网诊断与一键修复工具箱（DNS 刷新、IP 续租、IPv6、适配器重置等）
- 流量数据导出 CSV / JSON（含 OS 网卡级计数基线）
- 常驻占用优化（历史留存、电池降频、GeoIP 默认关闭）
- macOS 公证与多平台安装包（DMG / EXE / MSI / DEB / AppImage）
