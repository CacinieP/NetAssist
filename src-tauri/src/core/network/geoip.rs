...
pub async fn lookup_geoip(ip: &str) -> Option<GeoIPInfo> {
    // Check if it's a local/private IP (reuse platform::common)
    if let Ok(addr) = ip.parse::<std::net::IpAddr>() {
        if crate::platform::common::is_local_ip(&addr) {
...
async fn lookup_geoip_online(ip: &str) -> Result<GeoIPInfo, String> {
    ...
    for (idx, (base_url, user_agent)) in services.iter().enumerate() {
        ...
        let data = match resp.json::<serde_json::Value>().await {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to parse GeoIP JSON response from {}: {}",
                    base_url,
                    e
                );
                continue;
        ...
        match &result {
            ...
            Ok(geoip) => {
                tracing::debug!(
                    "GeoIP service {} returned: country='{}', region='{}', city='{}', trying next",
                    base_url,
                    geoip.country,
                    geoip.region,
                    geoip.city
            ...
            Err(e) => {
                tracing::warn!(
                    "GeoIP service {} returned error: {}, trying next",
                    base_url,
                    e
                );
            }
...
fn parse_ipapi_response(data: serde_json::Value) -> Result<GeoIPInfo, String> {
    ...
    if data["status"].as_str() != Some("success") {
        let message = data["message"]
            ...
            .to_string();
        tracing::warn!(
            "ip-api.com returned status='{}', message='{}",
            data["status"].as_str().unwrap_or("none"),
            message
        );
        return Ok(GeoIPInfo {
            country: "未知".to_string(),
    ...
    tracing::debug!(
        "ip-api.com parsed: country={}, region={}, city={}",
        country,
        region,
        city
    );
...
fn parse_ipwhois_response(data: serde_json::Value) -> Result<GeoIPInfo, String> {
    ...
    tracing::debug!(
        "ipwhois.app parsed: country={}, region={}, city={}",
        country,
        region,
        city
    );
...
fn parse_ipapico_response(data: serde_json::Value) -> Result<GeoIPInfo, String> {
    ...
    let country = data["country_name"]
        .as_str()
        .or_else(|| data["country"].as_str())
        .unwrap_or("-")
        .to_string();
    let region = data["region"]
        .as_str()
        .or_else(|| data["region_name"].as_str())
        .unwrap_or("-")
        .to_string();
    let city = data["city"].as_str().unwrap_or("-").to_string();

    tracing::debug!(
        "ipapi.co parsed: country={}, region={}, city={}",
        country,
        region,
        city
    );
